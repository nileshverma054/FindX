    ...
    try:
      ...
      for i,d in data[:10].iterrows():
        ...
        try:
          ...
          samples.append(d_hist)
          print(d_hist)
        except:
          pass
      cPickle.dump(data,open(os.path.join(cache_dir, sample_cache), "wb", True))
      data["hist"]=samples
      # data.to_csv("database/final_data.csv")
